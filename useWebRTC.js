...
export function useWebRTC({ roomId, userName }) {
  ...
  // ── Initiate call to a peer (caller side) ──────────────────────────────────
  const callPeer = useCallback(
    async (peerId, peerName) => {
      const pc = createPeerConnection(peerId, peerName);
      setPeers((prev) => ({ ...prev, [peerId]: { stream: null, name: peerName, video: true, audio: true } }));
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Joined room — call all existing participants
      ...
      socket.on("offer", async ({ offer, fromId, fromName }) => {
        ...
        if (!pc) {
          ...
          setPeers((prev) => ({ ...prev, [fromId]: { stream: null, name: fromName, video: true, audio: true } }));
        }
      ...
      socket.on("participant-media-state", ({ participantId, video, audio }) => {
        setPeers((prev) =>
          prev[participantId]
            ? { ...prev, [participantId]: { ...prev[participantId], video, audio } }
            : prev
        );
      });
